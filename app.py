...
from sagemaker.serializers import NumpSerializer
from sagemaker.deserializers import JSONeserializer
...
if len(uploaded_files) == 3:
    ...
    for file in uploaded_files:
        ...
        if file.name.endwith("vv.tif"):
            with rasterio.open(file) as vv:
                vv_img = vv.read(1)
                print(type(vv_img))
                print("Loaded vv image.")

        elif file.name.endwith("vh.tif"):
